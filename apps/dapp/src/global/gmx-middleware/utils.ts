...
export function shortenAddress(address: string, padRight = 4, padLeft = 6) {
  return (
    address.slice(0, padLeft) + '...' + address.slice(address.length - padRight, address.length)
  )
}
...
const readableLargeNumber = Intl.NumberFormat('en-US', {
  maximumFractionDigits: 0,
})
const readableSmallNumber = Intl.NumberFormat('en-US', {
  maximumFractionDigits: 2,
  minimumFractionDigits: 2,
})
const readableTinyNumber = Intl.NumberFormat('en-US', {
...
export function readableNumber(ammount: number | bigint) {
  const absAmount =
    typeof ammount === 'bigint' ? (ammount > 0n ? ammount : -ammount) : Math.abs(ammount)
...
export function formatReadableUSD(
  ...
  if (displayDecimals) {
    return currency + readableNumber(amountUsd)
  }

  return currency + readableNumber(amountUsd).replace(/\.\d+/, '')
}
...
export function intervalListFillOrderMap<T, R, RTime extends R & TimelineTime = R & TimelineTime>({
  ...
  if (seed.time > initialSourceTime) {
    throw new Error(
      `inital source time: ${initialSourceTime} must be greater then seed time: ${seed.time}}`,
    )
  }
...
export function pagingQuery<
  T,
  ReqParams extends IRequestPagePositionApi & (IRequestSortApi<T> | {}),
>(
  ...
  if ('selector' in queryParams) {
    const comperator =
      typeof customComperator === 'function' ? customComperator : defaultComperator(queryParams)
...
export const filterNull = <T>(prov: Stream<T | null>) =>
  filter((provider): provider is T => provider !== null, prov)
...
export function groupByMapMany<A, B extends string | symbol | number>(
  list: A[],
  getKey: (v: A) => B,
) {
...
export function getSafeMappedValue<T extends Object>(
  contractMap: T,
  prop: any,
...
export function decodeReferralCode(hexCode?: string) {
  if (!hexCode || hexCode === ARBITRUM_ADDRESS.HashZero) return ''
  try {
...
export function encodeReferralCode(code: string) {
  ...
  if (final.length > MAX_REFERRAL_CODE_LENGTH) {
    return ARBITRUM_ADDRESS.HashZero
  }
