...
const ImageRender = ({ value }: any) => {
  return (
    <img
      ...
      alt={value._key}
      src={GetThumbnailUrl(value.asset._ref)}
    />
  )
}
  ...
  return (
    <div>
      {value.children.map((child: any) =>
        formatText(child, value.markDefs, value.style)
      )}
    </div>
  )
}
...
const Post = (post: PostProps) => {
  return (
    <div className="mt-2">
      ...
      <h3 className="card-title md:truncate my-2 text-xs">
        {moment(post.pubDate).fromNow()}
      </h3>
      <img
        src={GetThumbnailUrl(post.mainImage)}
        alt={post.title}
