...
var require_filterDuplicates = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/filterDuplicates/filterDuplicates.js"(exports) {
    ...
    var filterDuplicates = (value, i, a) => a.findIndex((t) => t === value) === i;
    exports.filterDuplicates = filterDuplicates;
    var filterDuplicatesByKey = (key) => (value, i, a) => a.findIndex((t) => t[key] === value[key]) === i;
    exports.filterDuplicatesByKey = filterDuplicatesByKey;
  }
});
...
var require_isArray = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/isArray/isArray.js"(exports) {
    ...
    var isArrayNotEmpty = (array) => !!(array === null || array === void 0 ? void 0 : array.length);
...
var require_isTrue = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/isTrue/isTrue.js"(exports) {
    ...
    var isPropertyTrue = (property) => (value) => value[property] === true;
    exports.isPropertyTrue = isPropertyTrue;
...
var require_isTruthy = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/isTruthy/isTruthy.js"(exports) {
    ...
    var isTruthy4 = (value) => !!value;
    exports.isTruthy = isTruthy4;
    var isFalsy = (value) => !value;
    exports.isFalsy = isFalsy;
    var isPropertyTruthy = (property) => (value) => !!value[property];
    exports.isPropertyTruthy = isPropertyTruthy;
    var isPropertyFalsy = (property) => (value) => !value[property];
    exports.isPropertyFalsy = isPropertyFalsy;
...
var require_sorting = __commonJS({
  "../../node_modules/.pnpm/typesafe-utils@1.16.2/node_modules/typesafe-utils/lib/cjs/sorting/sorting.js"(exports) {
    ...
    var sortStringASC = (a, b) => sortASC(a === null || a === void 0 ? void 0 : a.toLowerCase(), b === null || b === void 0 ? void 0 : b.toLowerCase());
    exports.sortStringASC = sortStringASC;
    var sortStringPropertyASC = (property) => ({ [property]: a }, { [property]: b }) => exports.sortStringASC(a, b);
    exports.sortStringPropertyASC = sortStringPropertyASC;
...
var require_sync = __commonJS({
  "../../node_modules/.pnpm/tiny-glob@0.2.9/node_modules/tiny-glob/sync.js"(exports, module2) {
    ...
    var { join: join2, resolve: resolve5, relative } = require("path");
    var isHidden = /(^|[\\\/])\.[^\\\/\.]/g;
    var CACHE = {};
    function walk(output, prefix, lexer, opts, dirname2 = "", level = 0) {
      const rgx = lexer.segments[level];
      const dir = resolve5(opts.cwd, prefix, dirname2);
      const files = fs2.readdirSync(dir);
      const { dot, filesOnly } = opts;
      let i = 0, len = files.length, file;
      let fullpath, relpath, stats, isMatch;
      for (; i < len; i++) {
        fullpath = join2(dir, file = files[i]);
        relpath = dirname2 ? join2(dirname2, file) : file;
        ...
        if (rgx && !rgx.test(file))
          continue;
        !filesOnly && isMatch && output.push(join2(prefix, relpath));
    ...
    module2.exports = function(str, opts = {}) {
      ...
      if (!glob2.isGlob) {
        try {
          let resolved = resolve5(opts.cwd, str);
      ...
      return opts.absolute ? matches.map((x) => resolve5(opts.cwd, x)) : matches;
    };
  }
});
...
__export(src_exports, {
  readTranslationFromDisk: () => readTranslationFromDisk,
  readTranslationsFromDisk: () => readTranslationsFromDisk
});
...
`);
  ...
  const outputPathParts = (0, import_path3.resolve)(outputPath, locale).replace((0, import_path3.resolve)(), "").split(import_path3.sep).filter(import_typesafe_utils2.isTruthy);
...
var transpileTypescriptFiles = async (outputPath, outputFormat2, languageFilePath, locale, tempPath, typesFileName) => {
  ...
  return (0, import_path3.resolve)(tempPath, baseTranslationPath, "index.js");
};
var parseLanguageFile = async (outputPath, outputFormat2, typesFileName, tempPath, locale, namespace = "") => {
  ...
  const originalPath = (0, import_path3.resolve)(outputPath, fileName, `index${fileEnding}`);
...
var findAllNamespacesForLocale = (locale, outputPath) => {
  try {
    ...
    }).filter(import_typesafe_utils3.isTruthy);
  } catch (ignore) {
    return [];
  }
};
...
var readTranslation = async (locale, outputPath, outputFormat2, tempPath, typesFileName) => {
  logger2.info(`exporting translations for locale '${locale}' ...`);
  const translations = await parseLanguageFile(
    outputPath,
    outputFormat2,
    typesFileName,
    (0, import_path4.resolve)(tempPath, locale),
    locale
  );
  if (!translations) {
    logger2.error(`could not find locale file '${locale}'`);
    return { locale, translations: {}, namespaces: [] };
  }
  const namespaces = findAllNamespacesForLocale(locale, outputPath);
  for (const namespace of namespaces) {
    logger2.info(`adding namespace '${locale}/${namespace}' ...`);
    translations[namespace] = await parseLanguageFile(
      outputPath,
      outputFormat2,
      typesFileName,
      (0, import_path4.resolve)(tempPath, locale, namespace),
      locale,
      namespace
    ) || {};
  }
  logger2.info(`exporting translations for locale '${locale}' completed`);
  return { locale, translations, namespaces };
};
var readTranslationFromDisk = async (locale) => {
  const config = await setup();
  const { outputPath, outputFormat: outputFormat2, tempPath, typesFileName } = config;
  return await readTranslation(locale, outputPath, outputFormat2, tempPath, typesFileName);
};
var readTranslationsFromDisk = async () => {
  const config = await setup();
  const { outputPath, outputFormat: outputFormat2, tempPath, typesFileName } = config;
  const locales = await getAllLocales(import_promises.default, outputPath, outputFormat2);
  const promises = locales.map(
    (locale) => readTranslation(locale, outputPath, outputFormat2, tempPath, typesFileName)
  );
